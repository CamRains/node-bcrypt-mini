...
module.exports = {
  signup: (req, res) => {
    ...
    db.check_user_exists(email).then(user => {
      ...
      if (user.length) {
        //   if user exists and we will send a signup error?
        ...
        bcrypt.genSalt(saltRounds).then(salt => {
          // hash the plain text password send by user with the saltrounds to get the final hashed password
          bcrypt.hash(password, salt).then(hashedPassword => {
            //   after hashing is suxxesful we will store th eusers data in the db
